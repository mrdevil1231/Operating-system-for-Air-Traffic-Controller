...
void* departureThread(void* param)
{
  ...
  int res = checkRunway(runway);
  if(res==1)
  {
  sem_wait(&semaphore);
  sleep(2);
  if(runway!=-1){
  printf("\nPlane %d has completed boarding/loading and taken off from Runway No. %d of Airport No. %d.",dat.msg.Fd.planeID,runway,dat.msg.Fd.Departure_Code);
  }
  else
  {
   printf("\nPlane %d has completed boarding/loading and taken off from BackupRunway of Airport No. %d.",dat.msg.Fd.planeID,dat.msg.Fd.Departure_Code);
  }
  fflush(stdout);
  sem_post(&semaphore);
  }
